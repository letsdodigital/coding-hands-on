...
class QueryConsentForm:
    def __init__(self, main_table_name):
        ...
        self.main_table = execute_query(self.conn.table(main_table_name).select("*"), ttl="0")
        return
    ...
    def execute(self, foreign_table_name, field):
        ...
        """
        foreign_table_data_all = execute_query(self.conn.table(foreign_table_name).select("*"), ttl="0")
        ...
        if not main_table_data_single:
            raise ValueError("No matching data found in the main table after 5 attempts.")
...
def main():
    ...
    st.date_input(
        "Date of birth", value=date_of_birth_object, disabled=True
    )
    st.text_input("Email", value=patient["email"], disabled=True)

    st.header("Intervention")
    st.text_input(
        "Intervention", value=consent_type["type"], disabled=True
    )
    st.text_area(
        "Full description",
